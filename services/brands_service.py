...
def get(id):
    try:
        ...
        res = connection.execute(
            'SELECT * FROM brands WHERE id = ?', (id,)).fetchall()
...
def delete(id):
    try:
        ...
        with sqlite3.connect('database.db') as connection:
            connection.row_factory = dict_factory
        res = connection.execute(
            'DELETE FROM brands WHERE id = ? RETURNING *', (id, )).fetchall()
...
def update_status(id, status):
    try:
        ...
        print(status)
        res = connection.execute(
            'UPDATE brands SET status = ? WHERE id = ? RETURNING *', (status_list[status], id)).fetchall()
...
def get_by_name(name):
    try:
        with sqlite3.connect('database.db') as connection:
            connection.row_factory = dict_factory
        res = connection.execute(
            'SELECT * FROM brands WHERE name LIKE = ?', ('%' + name + '%'),).fetchall()
